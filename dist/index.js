...
const ListView = ({ columns, rows, selectable = false, multiSelect = true, selectedIds = [], sort, loading = false, emptyMessage = 'No data available', hoverable = true, clickableRows = false, className = '', showHeader = true, maxHeight, onSelectionChange, onRowClick, onRowDoubleClick, onSortChange, onColumnHeaderClick, onRowAction, }) => {
    ...
    const renderCell = (column, row, rowIndex) => {
        ...
        switch (column.type) {
            case 'enum':
                return (jsxRuntime.jsx(EnumField, { value: value, options: column.options || [], mode: "readonly", ...(column.fieldProps || {}) }));
            case 'name':
            case 'relate':
                return (jsxRuntime.jsx(NameField, { value: value, mode: "readonly", link: column.type === 'name' || column.type === 'relate', recordId: row.data[`${column.key}_id`], module: row.data[`${column.key}_module`], onLinkClick: column.fieldProps?.onLinkClick ||
                        ((recordId, module) => {
            ...
            case 'url':
            case 'text':
                return (jsxRuntime.jsx(TextField, { value: value, mode: "readonly", type: column.type === 'email' ? 'email' : column.type === 'phone' ? 'tel' : column.type === 'url' ? 'url' : 'text', ...(column.fieldProps || {}) }));
            case 'custom':
